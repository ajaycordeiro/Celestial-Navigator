...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      H2('Initial Build — Core App'),
      Bullet('Build me a full-stack astronomy web app called StarGazer. I want it to show real-time sky conditions from my location — planets, moon, bright stars, deep sky objects, ISS passes, weather. Dark theme, professional look, monospace font for the data.'),
      Bullet('Make the app location-aware. Add a location picker that lets me either auto-detect via GPS or search by city name. Save the location so it persists when I come back.'),
      Bullet('Add a Dashboard as the home page. Show a sky overview with current conditions, a highlight of what is visible tonight, and quick navigation cards to every section.'),
      Spacer(),

      H2('Initial Build — Individual Tabs'),
      Bullet('Add a Planets tab showing all eight planets with rise time, transit time, set time, current altitude, azimuth, magnitude, and angular diameter. Show which ones are above the horizon right now.'),
      Bullet('Add a Moon tab. I want to see the current phase name, illumination percentage, distance from Earth, angular diameter, rise and set times, and the dates of the next new moon and full moon.'),
      Bullet('Add a Stars tab showing the brightest stars. Include altitude, azimuth, magnitude, spectral type, distance in light years, and the constellation. Let me filter to only stars above the horizon.'),
      Bullet('Add a Deep Sky Objects tab with Messier and NGC objects — galaxies, nebulae, star clusters. Show visibility rating, angular size, distance, and observing notes for each object.'),
      Bullet('Add an ISS Tracker tab that shows the next visible passes of the International Space Station over my location. Include max elevation, direction, rise and set times, and pass duration.'),
      Bullet('Add a Conditions tab showing the sky quality for tonight. I want cloud cover percentage, atmospheric seeing index, transparency, wind speed, humidity, and an overall verdict like Good / Moderate / Poor.'),
